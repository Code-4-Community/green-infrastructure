...
/*-> Scan the entire GI Applications table and return all applications */
export async function scanAllGIApplications(): Promise<unknown[]> {
  const command = new ScanCommand({
  ...
  const unmarshalledItems = dynamoRawResult.Items.map((i) => unmarshall(i));
  console.log(unmarshalledItems);
  console.log('hit endpoint for GI Applications');
  return unmarshalledItems;
}

/*-> Scan GI Applications and grab the item that matches the id inside the filter */
export async function scanGIApplicationDetails(filter: ApplicationFilters): Promise<unknown> {
   
  // Assuming 'appID' is the name of the attribute and it's of type String
  const applicationID: number = filter.uniqueIdentifier;

  const command = new GetItemCommand({
    TableName: filter.tableName,
    Key: {
      'appID': {N: applicationID.toString() }
    }
  });
  
  const dynamoRawResult = await client.send(command);
  if (!dynamoRawResult || !dynamoRawResult.Item) {
    throw new Error('No item found with the given appID');
  }

  const unmarshalledItem = unmarshall(dynamoRawResult.Item);
  console.log(unmarshalledItem);
  console.log('Retrieved GI Application with appID:', filter.uniqueIdentifier);
