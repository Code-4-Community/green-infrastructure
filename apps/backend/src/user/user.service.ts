...
export class UserService {
  ...
  public async postUser(userData: NewUserInput, role: Role) {
    const userModel = this.postInputToUserModel(userData, role);
    const newId =
   ...
   * @returns the user's information as a UserModel object
   */
  private mapDynamoDBItemToUserModel(
    objectId: number,
    data: { [key: string]: any },
  ): UserModel {
  ...
  private PostInputToUserVolunteerModel = (
    input: NewUserInput,
  ): UserInputModel => {
    return {
      userId: { N: '0' },
